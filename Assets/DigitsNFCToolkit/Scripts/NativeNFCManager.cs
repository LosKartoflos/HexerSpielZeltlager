...
namespace DigitsNFCToolkit
{
	...
	public class NativeNFCManager: MonoBehaviour
	{
		...
		/// <summary>Checks if NDEF Push is supported on this device</summary>
		public static bool IsNDEFPushSupported()
		{
			NativeNFC nativeNFC = Instance.nfc;
			return nativeNFC.IsNDEFPushSupported();
		}
		...
		/// <summary>(iOS only) Sets the alert message when doing a read operation</summary>
		public static void SetReadAlertMessage(string alertMessage)
		{
			NativeNFC nativeNFC = Instance.nfc;
			nativeNFC.SetReadAlertMessage(alertMessage);
		}

		/// <summary>(iOS only) Sets the alert message when doing a write operation</summary>
		public static void SetWriteAlertMessage(string alertMessage)
		{
			NativeNFC nativeNFC = Instance.nfc;
			nativeNFC.SetWriteAlertMessage(alertMessage);
		}
	}
}
